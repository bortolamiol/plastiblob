-----------------------------------------------------------------------------------------
--
-- SECONDO LIVELLO DEL GIOCO: SALTARE E SPARARE AI NEMICI E RACCOGLIERE LA PLASTICA DAL CIELO
--
-----------------------------------------------------------------------------------------
...
local musicLevel2
local explosionSound2 = audio.loadSound("MUSIC/explosion.mp3") --carico suono esplosione
...
function scene:show( event )
  ...
  elseif phase == "did" then
    ...
    local options = {
      effect = "fade",
      time = 1000,
      params = { level="level2"} -- questa variabile verrà utilizzata dal gameover pper capire a che livello tornare
    }
    ...
    bg[1] = display.newImageRect("immagini/livello-2/background.png", _w, _h)
    bg[1].anchorY = 0
    bg[1].anchorX = 0
    bg[1].x = 0
    bg[1].y = _y
    group_background:insert(bg[1])
    bg[2] = display.newImageRect("immagini/livello-2/background.png", _w, _h)
    bg[2].anchorY = 0
    bg[2].anchorX = 0
    bg[2].x = _w
    bg[2].y = _y
    ...
    local enemyWalkingSheetData = { width=300, height=200, numFrames=8, sheetContentWidth=2400, sheetContentHeight=200 }
    local enemyWalkingSheet = graphics.newImageSheet( "immagini/livello-2/serpentaccio.png", enemyWalkingSheetData )
    local enemyData = {
    ...
    -- Sacchetto in plastica che vola per lo schermo
    ...
    ----------------------------------------------------------------------------
    ----------------------------------------------------------------------------
    --------------- FUNZIONI PER IL SECONDO NEMICO DEL GIOCO -------------------
    ----------------------------------------------------------------------------
    ----------------------------------------------------------------------------
    ...
    function sprite.collision( self, event )
      if( event.phase == "began" ) then
        ...
        if(event.other.name ==  "plasticbag") then --mi sono scontrato con il sacchetto
          ...
          scoreText.text = scoreCount.."/10" --aggiorno il testo dei numeri di oggetti di plastica raccolti
    ...
    function onBulletCollision( event ) --controlla se ci state collisioni del proiettile con il nemico
      ...
      if(tostring(event.other.name) == "enemy") then --se il proiettile si è scontrato contro un nemico allora..
        ...
        explosion:play()
        audio.play( explosionSound2 ) --faccio partire audio esplosione
